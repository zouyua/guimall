...
import com.gg.guimall.admin.service.StockWarningService;
...
public class StockWarningServiceImpl implements StockWarningService {
    ...
    @Override
    public void checkProductStockWarning(Long productId) {
        ...
        try {
            ...
            for (PmsSkuStockDO skuStock : skuList) {
                checkStockWarning(skuStock.getId());
            }
        } catch (Exception e) {
            log.error("批量检查库存预警异常, productId={}", productId, e);
        }
    }
}
